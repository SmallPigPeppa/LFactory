...
class PackedSupervisedDatasetProcessor(SupervisedDatasetProcessor):
    def preprocess_dataset(self, examples: dict[str, list[Any]]) -> dict[str, list[Any]]:
        valid_num = 0
        batch_input_ids, batch_labels, batch_images = [], [], []
        lengths = []
        ...
        for knapsack in knapsacks:
            ...
            for subseq_idx, length in enumerate(knapsack):
                ...
                if self.data_args.neat_packing:
                    ...
                    packed_attention_masks += [subseq_idx + 1] * len(batch_input_ids[index])
                else:
                    packed_attention_masks += [1] * len(batch_input_ids[index])

            right_padding_length = self.data_args.cutoff_len - len(packed_input_ids)
            if right_padding_length > 0:
                packed_input_ids += [self.tokenizer.pad_token_id] * right_padding_length
                packed_position_ids += [0] * right_padding_length
                packed_labels += [IGNORE_INDEX] * right_padding_length
                packed_attention_masks += [0] * right_padding_length

            if len(packed_input_ids) != self.data_args.cutoff_len:
                raise ValueError("The length of packed example should be identical to the cutoff length.")
            if self.data_args.neat_packing and packed_attention_masks.count(0) != right_padding_length:
                raise ValueError("The number of padding tokens should be identical to the right padding length.")
            ...
            model_inputs["packing_params"].append(
                asdict(PackingParams(sequence_boundaries, image_subseq_ids, right_padding_length))
                if self.data_args.neat_packing
                else None
            )
        return model_inputs
